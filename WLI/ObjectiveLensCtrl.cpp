...
void CObjectiveLensCtrl::DrawLens(Graphics& g, RectF bounds, CString label, BOOL isSelected)
{
    ...
    if (label.Find(_T("2x")) != -1) cStripe = Color(255, 201, 0, 2); // Red
    if (label.Find(_T("5x")) != -1) cStripe = Color(255, 254, 155, 74); // Yellow
    if (label.Find(_T("10x")) != -1) cStripe = Color(255, 205, 193, 1); // Green
    if (label.Find(_T("20x")) != -1) cStripe = Color(255, 59, 155, 27); // Blue
    if (label.Find(_T("50x")) != -1) cStripe = Color(255, 1, 150, 250); // White/Gray
