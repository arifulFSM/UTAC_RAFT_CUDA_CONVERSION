...
namespace CAM {
	class CBaslerColourCam : public CCamera
	...
	public:
		...
		SCtx Ctx;
		void DisplayImage();
		void ExecuteTriggerLoc();
		CameraProp* camProp = NULL;
		void propertyPageDisplay();
		BOOL ExecuteTrigger();
		BOOL SetTriggerMode(bool isTrigOn);
		BOOL GetBitmapImage(IMGL::CIM& Im);
		float piCurPos = -1;
	};
}

// extern CAM::CBaslerColourCam BasCol;
