...
void CStripDlg::UpdateImage(int idx) {
	...
	if (ICC.isRegionType == ICC.AREA) { // Area Region - 07122023
		ICC.x3 = min(ICC.x1, ICC.x2);
		ICC.y3 = max(ICC.y1, ICC.y2);
		ICC.x4 = max(ICC.x1, ICC.x2);
		ICC.y4 = min(ICC.y1, ICC.y2);
		ImT.DrawLine(ICC.x1, ICC.y1, ICC.x4, ICC.y4, 0, 0, 0);
		ImT.DrawLine(ICC.x1, ICC.y1, ICC.x3, ICC.y3, 0, 0, 0);
		ImT.DrawLine(ICC.x3, ICC.y3, ICC.x2, ICC.y2, 0, 0, 0);
		ImT.DrawLine(ICC.x4, ICC.y4, ICC.x2, ICC.y2, 0, 0, 0);
	}
	else {
		if (ICC.isOriental == ICC.HORIZONTAL) ImT.DrawLine(ICC.x1, ICC.y1, ICC.x2, ICC.y1, 0, 0, 0);
		else ImT.DrawLine(ICC.x1, ICC.y1, ICC.x1, ICC.y2, 0, 0, 0);
	}
...
BEGIN_MESSAGE_MAP(CStripDlg, CResizableDialog)
	...
	ON_MESSAGE(WL_LMOUSEDOWN, &CStripDlg::OnWlMouseDown) // 05302023 - Mortuja
	ON_MESSAGE(WL_LMOUSEUP, &CStripDlg::OnWlMouseUp) // 05302023 - Mortuja
	...
	ON_BN_CLICKED(IDC_RAD_AREA, &CStripDlg::OnBnClickedRadArea)
	ON_BN_CLICKED(IDC_RAD_LINE, &CStripDlg::OnBnClickedRadLine)
	ON_BN_CLICKED(IDC_RAD_HORIZON, &CStripDlg::OnBnClickedRadHorizon)
	ON_BN_CLICKED(IDC_RAD_VERTIC, &CStripDlg::OnBnClickedRadVertic)
	//ON_BN_CLICKED(IDC_ENABLE_DISABLE_HFUNC, &CStripDlg::OnBnClickedEnableDisableHfunc)
END_MESSAGE_MAP()
...
BOOL CStripDlg::OnInitDialog() {
	...
	AddAnchor(IDC_REGION_TYPE, TOP_RIGHT);
	AddAnchor(IDC_RAD_AREA, TOP_RIGHT);
	AddAnchor(IDC_RAD_LINE, TOP_RIGHT);
	AddAnchor(IDC_DIR_TYPE, TOP_RIGHT);
	AddAnchor(IDC_RAD_HORIZON, TOP_RIGHT);
	AddAnchor(IDC_RAD_VERTIC, TOP_RIGHT);
	//AddAnchor(IDC_ENABLE_DISABLE_HFUNC, TOP_RIGHT);
	...
	((CButton*)GetDlgItem(IDC_RAD_LINE))->SetCheck(true);
	((CButton*)GetDlgItem(IDC_RAD_HORIZON))->SetCheck(true);
	ICC.isOriental = ICC.HORIZONTAL;
	ICC.isRegionType = ICC.LINE; // 07122023
...
afx_msg LRESULT CStripDlg::OnWlMousemove(WPARAM wParam, LPARAM lParam) {
	//return TRUE; // 06222023
	if (ICC.isHeightMapPlot) return TRUE; // 05302023 - Mortuja
...
afx_msg LRESULT CStripDlg::OnWlMouseDown(WPARAM wParam, LPARAM lParam) {
	MTH::SPointf* pPt = (MTH::SPointf*)lParam;
	if ((pPt->x < 0) || (pPt->x > 1.0f) || (pPt->y < 0) || (pPt->y > 1.0f)) return 0;
...
afx_msg LRESULT CStripDlg::OnWlMouseUp(WPARAM wParam, LPARAM lParam) {
	MTH::SPointf* pPt = (MTH::SPointf*)lParam;
	if ((pPt->x < 0) || (pPt->x > 1.0f) || (pPt->y < 0) || (pPt->y > 1.0f)) return 0;
...
void CStripDlg::OnBnClickedButton8() {
	...
	if (Strip.GenHMapV5(Rcp) && hWndParent) {
		...
		::PostMessageW(hWndParent, IDC_HEIGHT_MAP_PM, 0, 0); // 05302023 - Mortuja
		::PostMessageW(hWndParent, IDC_ROUGHNESS_DLG, 0, 0); // 05302023 - Mortuja
	}
	Strip.AddTime(CHighTime::GetPresentTime());
...
void CStripDlg::OnBnClickedRadArea() { // 07122023
	// TODO: Add your control notification handler code here
	ICC.isRegionType = ICC.AREA;
	ICC.isOriental = ICC.HORIZONTAL;
	GetDlgItem(IDC_RAD_VERTIC)->EnableWindow(0);
	GetDlgItem(IDC_RAD_HORIZON)->EnableWindow(0);
	((CButton*)GetDlgItem(IDC_RAD_HORIZON))->SetCheck(true);
	((CButton*)GetDlgItem(IDC_RAD_VERTIC))->SetCheck(false);
}

void CStripDlg::OnBnClickedRadLine() { // 07122023
	// TODO: Add your control notification handler code here
	ICC.isRegionType = ICC.LINE;
	GetDlgItem(IDC_RAD_VERTIC)->EnableWindow();
	GetDlgItem(IDC_RAD_HORIZON)->EnableWindow();
	((CButton*)GetDlgItem(IDC_RAD_HORIZON))->SetCheck(true);
	((CButton*)GetDlgItem(IDC_RAD_VERTIC))->SetCheck(false);
}

void CStripDlg::OnBnClickedRadHorizon() {
	// TODO: Add your control notification handler code here
	ICC.isOriental = ICC.HORIZONTAL;
}

void CStripDlg::OnBnClickedRadVertic() {
	// TODO: Add your control notification handler code here
	ICC.isOriental = ICC.VERTICAL;
}

// 05302023 - Mortuja [END]

//void CStripDlg::OnBnClickedEnableDisableHfunc() {
//	// TODO: Add your control notification handler code here
//	ICC.isHeightFuncEnable = !ICC.isHeightFuncEnable;
//	if (ICC.isHeightFuncEnable) {
//		GetDlgItem(IDC_ENABLE_DISABLE_HFUNC)->SetWindowTextW(L"Disable");
//	}
//	else {
//		GetDlgItem(IDC_ENABLE_DISABLE_HFUNC)->SetWindowTextW(L"Enable");
//	}
//}