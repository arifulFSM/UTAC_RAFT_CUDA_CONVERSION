...
BEGIN_MESSAGE_MAP(CBottomTabCtrl, CXTabCtrl)
    ON_WM_ERASEBKGND()
    ON_WM_MOUSEMOVE()
    ON_WM_MOUSELEAVE()
    ON_WM_PAINT()
...
void CBottomTabCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    CXTabCtrl::OnMouseMove(nFlags, point);
}
...
void CBottomTabCtrl::AdjustRect(BOOL bLarger, LPRECT lpRect)
{
    ...
    if (!bLarger && lpRect)
    {
        ...
        lpRect->top -= 20;   // Push top up
        lpRect->left -= 20;  // Push left out
        lpRect->right += 20; // Push right out
