...
void CFFT::fft_double2(unsigned int p_nSamples, bool p_bInverseTransform, double* pdata)
{
	unsigned int NumBits;
	unsigned int i, j, k, n;
	unsigned int BlockSize, BlockEnd;
	...
	double* p_lpRealIn = new double[p_nSamples];
	if (!p_lpRealIn) return;
	double* p_lpRealOut = new double[p_nSamples];
	if (!p_lpRealOut) return;
	double* p_lpImagOut = new double[p_nSamples];
	if (!p_lpImagOut) return;
	if (p_bInverseTransform) angle_numerator = -angle_numerator;

	for (i = 0;i < p_nSamples;i++)
		p_lpRealIn[i] = pdata[i];
	...
	for (i = 0; i < p_nSamples; i++)
	{
		j = ReverseBits(i, NumBits);
		p_lpRealOut[j] = p_lpRealIn[i];
		p_lpImagOut[j] = 0.0;//(p_lpImagIn == 0/*NULL*/) ? 0.0 : p_lpImagIn[i];
	}
	...
	for (i = 0; i < p_nSamples / 2; i++)
	{
		pdata[2 * i] = p_lpRealOut[i];
		pdata[2 * i + 1] = p_lpImagOut[i];
	}
	if (p_lpRealIn)
		delete[] p_lpRealIn;
	if (p_lpRealOut)
