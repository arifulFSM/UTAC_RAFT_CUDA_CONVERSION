...
IMPLEMENT_DYNAMIC(CInPEdit, CEdit)

CInPEdit::CInPEdit(int iItem, int iSubItem, CString sInitText) {
	m_iItem = iItem;
	m_iSubItem = iSubItem;
	m_bESC = FALSE;
...
BEGIN_MESSAGE_MAP(CInPEdit, CEdit)
	...
	ON_WM_CHAR()
	ON_WM_CREATE()
	ON_CONTROL_REFLECT(EN_KILLFOCUS, &CInPEdit::OnEnKillfocus)
END_MESSAGE_MAP()
...
int CInPEdit::OnCreate(LPCREATESTRUCT lpCreateStruct) {
...
void CInPEdit::OnEnKillfocus() {
	CWnd* pWndViewAttachmentsDlg = GetParent()->GetParent();
	if (pWndViewAttachmentsDlg) {
		pWndViewAttachmentsDlg->SendMessage(WM_NOTIFY_KILL_FOCUS,
