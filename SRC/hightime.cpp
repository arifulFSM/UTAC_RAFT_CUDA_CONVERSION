...
int CHighTime::FindStr(TCHAR* pszSource, const TCHAR* pszSub, size_t nStart) {
	size_t nLength = _tclen(pszSource);
	if (nStart > nLength)
		return -1;

	// find first
	TCHAR* pszSearch = _tcsstr(pszSource + nStart, pszSub);

	// return -1 for not found
	return (pszSearch == NULL) ? -1 : (int)(pszSearch - pszSource);
} // CHighTime::FindStr()

int CHighTime::ReplaceStr(TCHAR* pszDest, int nDestSize, const TCHAR* pszMask, const TCHAR* pszSource) {
	int nCount;
	size_t iInsertLen, iReplaceLen, iDestLen, iMoveLen;
	TCHAR* pSearch, * pStartPos;
...
// Formatting, Non MFC
LPCTSTR CHighTime::Format(TCHAR* pBuffer, int iBufferLen, const TCHAR* pFormat) const {
	...
	iPos = FindStr(pBuffer, _T("%M"), 0);
	if (iPos >= 0) {
		wsprintf(szTemp, _T("%03d"), tmHighTimeTemp.nMinute);
		ReplaceStr(pBuffer, iBufferLen, _T("%s"), szTemp);
	}

	iPos = FindStr(pBuffer, _T("%S"), 0);
	if (iPos >= 0) {
		wsprintf(szTemp, _T("%03d"), tmHighTimeTemp.nSecond);
		ReplaceStr(pBuffer, iBufferLen, _T("%s"), szTemp);
	}
...
CString CHighTime::Format(LPCTSTR lpszFormat) const {
	...
	//%u for the microsecond part
	//%n for the nanosecond part
	iPos = szParse.Find(L"%M", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", (tmHighTimeTemp.nMinute));
		szParse.Replace(L"%s", szTemp);
	}
	iPos = szParse.Find(L"%S", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", (tmHighTimeTemp.nSecond));
		szParse.Replace(L"%s", szTemp);
	}
	iPos = szParse.Find(L"%s", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", (tmHighTimeTemp.nMilli));
		szParse.Replace(L"%s", szTemp);
	}
	iPos = szParse.Find(L"%#s", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", (tmHighTimeTemp.nMilli));
		szParse.Replace(L"%#s", szTemp);
	}
	iPos = szParse.Find(L"%u", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", (tmHighTimeTemp.nMicro));
		szParse.Replace(L"%u", szTemp);
	}
	iPos = szParse.Find(L"%#u", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", (tmHighTimeTemp.nMicro));
		szParse.Replace(L"%#u", szTemp);
	}
	iPos = szParse.Find(L"%n", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", (tmHighTimeTemp.nNano));
		szParse.Replace(L"%n", szTemp);
	}
	iPos = szParse.Find(L"%#n", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", (tmHighTimeTemp.nNano));
		szParse.Replace(L"%#n", szTemp);
	}
	...
	// Fill in the buffer, disregard return value as it's not necessary
	strftime(CT2A(strDate.GetBuffer(MAX_TIME_BUFFER_SIZE)), MAX_TIME_BUFFER_SIZE,
		CT2A(szParse.GetBuffer(szParse.GetLength())), &tmTemp);
...
// formatting
TCHAR* CHighTimeSpan::Format(TCHAR* pBuffer, int iBufferLen, const TCHAR* pFormat) const {
	...
	iPos = CHighTime::FindStr(pBuffer, _T("%M"), 0);
	if (iPos >= 0) {
		wsprintf(szTemp, _T("%d"), LLABS(GetMinutes()));
		CHighTime::ReplaceStr(pBuffer, iBufferLen, _T("%M"), szTemp);
	}
	iPos = CHighTime::FindStr(pBuffer, _T("%S"), 0);
	if (iPos >= 0) {
		wsprintf(szTemp, _T("%d"), LLABS(GetSeconds()));
...
CString CHighTimeSpan::Format(LPCTSTR pFormat) const {
	...
	//%u for the microsecond part
	//%n for the nanosecond part
	iPos = szParse.Find(L"%D", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%0d", LLABS(GetDays()));
		szParse.Replace(L"%D", szTemp);
	}
	iPos = szParse.Find(L"%M", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", LLABS(GetMinutes()));
		szParse.Replace(L"%M", szTemp);
	}
	iPos = szParse.Find(L"%S", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", LLABS(GetSeconds()));
		szParse.Replace(L"%S", szTemp);
	}
	iPos = szParse.Find(L"%s", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", LLABS(GetMilliSeconds()));
		szParse.Replace(L"%s", szTemp);
	}
	iPos = szParse.Find(L"%#s", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", LLABS(GetMilliSeconds()));
		szParse.Replace(L"%#s", szTemp);
	}
	iPos = szParse.Find(L"%u", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", LLABS(GetMicroSeconds()));
		szParse.Replace(L"%u", szTemp);
	}
	iPos = szParse.Find(L"%#u", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", LLABS(GetMicroSeconds()));
		szParse.Replace(L"%#u", szTemp);
	}
	iPos = szParse.Find(L"%n", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%03d", LLABS(GetNanoSeconds()));
		szParse.Replace(L"%n", szTemp);
	}
	iPos = szParse.Find(L"%#n", 0);
	if (iPos >= 0) {
		szTemp.Format(L"%d", LLABS(GetNanoSeconds()));
		szParse.Replace(L"%#n", szTemp);
	}
	...
	// Fill in the buffer, disregard return value as it's not necessary
	strftime(CT2A(strSpan.GetBuffer(MAX_TIME_BUFFER_SIZE)), MAX_TIME_BUFFER_SIZE,
		CT2A(szParse.GetBuffer(szParse.GetLength())), &tmTemp);
	szParse.ReleaseBuffer();
...
CString CHighTimeSpan::FormatA(LPCTSTR pFormat) const {
	CString szParse(pFormat);
	CString szTemp, strSpan;
	CHighTime::_HighTimeFormat tmHighTimeTemp;
	int iPos;
