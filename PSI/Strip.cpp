...
//20250916
void WLI::CStrip::writeMatToCSV(const cv::Mat& mat, const std::string& filename)
{
	...
	if (mat.type() != CV_32F && mat.type() != CV_64F) {
		std::cerr << "Error: writeMatToCSV() only supports CV_32F or CV_64F matrices.\n";
		return;
	}
	...
	for (int i = 0; i < mat.rows; ++i) {
		const float* rowPtr = mat.ptr<float>(i);
...
//20250916
bool WLI::CStrip::GenHMapV5CVCUDA(RCP::SRecipe& Rcp) {
	...
	//	else ICC.x2 = ICC.x1;
	//}
	//float* pHeightDataGPU = new float[sz];
...
bool WLI::CStrip::GenHMapV5CV(RCP::SRecipe& Rcp) {
	...
	//float* pPHS1 = new float[sz];
	//float* pVIS1 = new float[sz];
	//std::ofstream fileP("C:\\WLIN\\DATA\\KernelData\\PHSCPU.csv");
	//std::ofstream fileV("C:\\WLIN\\DATA\\KernelData\\VISCPU.csv");
...
#pragma omp parallel for
		for (int y = 0; y < ht; y++) { // 05302023 - Mortuja
			...
			for (int x = 0; x < wd; x++) { // 05302023 - Mortuja
				...
				/*if (F.PhasePV5Test(Ch, WLI::PHS1, PsP, R2)) {
					Rsl = F.PeakPhas(WLI::PHS1, idx - 2, idx + 2, bPChg, sz);
				}*/
				else Rsl = BADDATA;
				//Im16um.SetPixel(x, y, Rsl);
